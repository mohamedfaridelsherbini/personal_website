...
> A ready-to-use Declarative pipeline lives in `Jenkinsfile`. Configure a JDK 21 tool named `jdk-21` in Jenkins, upload `.deploy.sh` (and optionally `.deploy.env`) as secret-file credentials, set their IDs via the environment variables `DEPLOY_SCRIPT_CREDENTIAL_ID` and `DEPLOY_ENV_CREDENTIAL_ID`, and check the `RUN_DEPLOY` parameter when you want Jenkins to run the same script after packaging. The pipeline copies those files into the workspace just before executing `./.deploy.sh`, so droplet-specific values stay outside the public repo while Jenkins reuses your local workflow.
